...
void LCD_Address_Set(u16 x1,u16 y1,u16 x2,u16 y2)
{
	if(USE_HORIZONTAL==0)
	{
		tft_send_command(tft_spi_device,0x2a);//列地址设置
		LCD_WR_DATA(x1+52);
		LCD_WR_DATA(x2+52);
		tft_send_command(tft_spi_device,0x2b);//行地址设置
		LCD_WR_DATA(y1+40);
		LCD_WR_DATA(y2+40);
		tft_send_command(tft_spi_device,0x2c);//储存器写
	}
...
{         
    ...
    {       
        ...
        if(y>LCD_H-16){y=x=0;tft_fill(RED);}
...
void spi_tft_init(void)
{
	...
	/*清屏*/
	tft_fill(WHITE);
	LCD_ShowString(0,0,"Hello,World!",RED);
}
...
{
	spi_tft_init(); //初始化显示屏
	LCD_ShowString(0,0,"Hello,Xiaofei!",RED);
	return 0;
}
...
static int tft_write(struct file *filp, const char __user *buf, size_t cnt, loff_t *off)
{
	...
	/*申请内存*/
	tft_display_struct *write_data;
	write_data = (tft_display_struct*)kzalloc(cnt, GFP_KERNEL);

	copy_number = copy_from_user(write_data, buf,cnt);
	LCD_ShowString(write_data->x, write_data->y, write_data->ch, write_data->color);
...
static int tft_probe(struct spi_device *spi)
{
	...
	tft_spi_device->max_speed_hz = 2000000;
