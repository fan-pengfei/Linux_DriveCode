...
struct led_resource
{
	struct device_node *device_node; //rgb_led_xiaofei的设备树节点
...
struct led_resource led_xiaofei;
...
static ssize_t led_chr_dev_write(struct file *filp, const char __user *buf, size_t cnt, loff_t *offt)
{
	...
	if (write_data & 0x04)
	{
		register_data = readl(led_xiaofei.virtual_DR);
		register_data &= ~(0x01 << 2);
		writel(register_data, led_xiaofei.virtual_DR); // GPIO1_04引脚输出低电平，红灯亮
	...
	else
	{
		register_data = readl(led_xiaofei.virtual_DR);
		register_data |= (0x01 << 2);
		writel(register_data, led_xiaofei.virtual_DR); // GPIO1_04引脚输出高电平，红灯灭
...
static int led_probe(struct platform_device *pdv)
{
	...
	/*获取rgb_led节点的红灯子节点*/
	led_xiaofei.device_node = of_find_node_by_name(rgb_led_device_node,"rgb_led_xiaofei");
	if (led_xiaofei.device_node == NULL)
	{
		printk(KERN_ERR "\n get rgb_led_xiaofei_device_node failed ! \n");
		return -1;
	}


	/*获取 reg 属性并转化为虚拟地址*/
	led_xiaofei.virtual_CCM_CCGR = of_iomap(led_xiaofei.device_node, 0);
	led_xiaofei.virtual_IOMUXC_SW_MUX_CTL_PAD = of_iomap(led_xiaofei.device_node, 1);
	led_xiaofei.virtual_IOMUXC_SW_PAD_CTL_PAD = of_iomap(led_xiaofei.device_node, 2);
	led_xiaofei.virtual_DR = of_iomap(led_xiaofei.device_node, 3);
	led_xiaofei.virtual_GDIR = of_iomap(led_xiaofei.device_node, 4);

	/*初始化红灯*/
	register_data = readl(led_xiaofei.virtual_CCM_CCGR);
	register_data |= (0x03 << 26);
	writel(register_data, led_xiaofei.virtual_CCM_CCGR); //开启时钟

	register_data = readl(led_xiaofei.virtual_IOMUXC_SW_MUX_CTL_PAD);
	register_data &= ~(0xf << 0);
	register_data |= (0x05 << 0);
	writel(register_data, led_xiaofei.virtual_IOMUXC_SW_MUX_CTL_PAD); //设置复用功能

	register_data = readl(led_xiaofei.virtual_IOMUXC_SW_PAD_CTL_PAD);
	register_data = (0x10B0);
	writel(register_data, led_xiaofei.virtual_IOMUXC_SW_PAD_CTL_PAD); //设置PAD 属性

	register_data = readl(led_xiaofei.virtual_GDIR);
	register_data |= (0x01 << 2);
	writel(register_data, led_xiaofei.virtual_GDIR); //设置GPIO1_04 为输出模式

	register_data = readl(led_xiaofei.virtual_DR);
	register_data |= (0x01 << 2);
	writel(register_data, led_xiaofei.virtual_DR); //设置 GPIO1_04 默认输出高电平
...
static void __exit led_platform_driver_exit(void)
{
	...
	iounmap(led_xiaofei.virtual_CCM_CCGR);
	iounmap(led_xiaofei.virtual_IOMUXC_SW_MUX_CTL_PAD);
	iounmap(led_xiaofei.virtual_IOMUXC_SW_PAD_CTL_PAD);
	iounmap(led_xiaofei.virtual_DR);
	iounmap(led_xiaofei.virtual_GDIR);
