...
int oled_res_pin_number;			   // 保存oled D/C控制引脚的引脚号（从对应设备树节点中获取）
u8 oled_init_data[] = {
...
{
	/*初始化oled*/
	gpio_direction_output(oled_res_pin_number, 0);
msleep(100);
	gpio_direction_output(oled_res_pin_number, 1);
msleep(100);
...
static int oled_probe(struct spi_device *spi)
{
	...
	oled_res_pin_number = of_get_named_gpio(oled_device_node, "res_pin", 0);
	printk("oled_control_pin_number = %d,\n ", oled_res_pin_number);
	gpio_direction_output(oled_res_pin_number, 1);
	/*初始化spi*/
	oled_spi_device = spi;
